<div class="w-full" id="reservation-form-root">
    ...
    <form wire:submit.prevent="submit" id="lf-reservation-form" novalidate class="flex flex-col">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-4">
            ...
            {{-- Full Name --}}
            <div class="lf-field-wrap light-mode @if($name) has-value @endif" id="wrap-name">
                ...
                @error('name')
                    <p class="font-mono text-xs font-bold mt-2 text-red-500">{{ $message }}</p>
                @enderror
            </div>

            {{-- Email --}}
            <div class="lf-field-wrap light-mode @if($email) has-value @endif" id="wrap-email">
        ...
        {{-- Row 2: Phone --}}
        <div>
            <div class="lf-field-wrap light-mode @if($phone) has-value @endif" id="wrap-phone">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-4">
            ...
            {{-- Date --}}
            <div class="lf-field-wrap light-mode @if($date) has-value @endif" id="wrap-date">
            ...
            <div class="lf-field-wrap light-mode @if($time) has-value @endif select-none" id="wrap-time">
                ...
                <div class="relative w-full">
                    <button
                        ...
                        style="color: {{ $time ? '#eba13d' : '#efe1d9' }}; cursor: pointer;"
                    >
                        <span class="flex-1 text-center pl-8">{{ $time ? \Carbon\Carbon::createFromFormat('H:i', $time)->format('g:i A') : 'Select Arrival Time' }}</span>
                    ...
                    <div id="time-dropdown-list" class="absolute left-0 right-0 mt-2 max-h-60 overflow-y-auto bg-[#020b0a] border border-[#eba13d]/25 rounded-lg z-50 hidden shadow-[0_20px_50px_rgba(0,0,0,0.6)]">
                        @foreach(['08:00','08:30','09:00','09:30','10:00','10:30','11:00','11:30','12:00','12:30','13:00','13:30','14:00','14:30','15:00','15:30','16:00','16:30','17:00','17:30','18:00','18:30','19:00','19:30','20:00','20:30','21:00'] as $t)
                            <button
                                type="button"
                                wire:click="$set('time', '{{ $t }}')"
                                ...
                                style="color: {{ $time === $t ? '#eba13d' : '#efe1d9' }};"
                            >
        ...
        <div class="border-y border-white/10 party-size-container text-center flex flex-col items-center" style="margin-top: 80px; margin-bottom: 80px; padding-top: 24px; padding-bottom: 24px;">
            <div class="font-mono text-sm uppercase tracking-[0.2em] text-[#eba13d] font-bold mb-8">
                Party Size ({{ $party_size }} {{ $party_size === 1 ? 'Guest' : 'Guests' }})
            </div>
            <div class="flex flex-wrap gap-6 justify-center max-w-2xl w-full" role="group" aria-label="Select party size">
                @foreach(range(1, 10) as $n)
                    <button
                        type="button"
                        wire:click="$set('party_size', {{ $n }})"
                        class="party-btn {{ $party_size === $n ? 'active-party' : '' }}"
                        aria-pressed="{{ $party_size === $n ? 'true' : 'false' }}"
                ...
                <button
                    ...
                    class="party-btn {{ $party_size > 10 ? 'active-party' : '' }}"
                    style="width: 72px; border-radius: 9999px;"
                    aria-pressed="{{ $party_size > 10 ? 'true' : 'false' }}"
            ...
            @error('party_size')
                <p class="font-mono text-xs font-bold mt-4 text-red-500">{{ $message }}</p>
            @enderror
        </div>

        {{-- Special Requests --}}
        <div style="margin-bottom: 80px;">
            <div class="lf-field-wrap light-mode @if($special_requests) has-value @endif" id="wrap-requests">
